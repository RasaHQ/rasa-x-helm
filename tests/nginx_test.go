...
func TestParameters_Nginx(t *testing.T) {
	t.Parallel()
	...
	optionsWithValues := &helm.Options{
		SetValues: map[string]string{
			"nginx.podLabels.test_label": "test_value",
		},
	...
	outputWithOptions := helm.RenderTemplate(t, optionsWithValues, helmChartPath, releaseName, []string{"templates/nginx-deployment.yaml"})
	outputWithoutOptions := helm.RenderTemplate(t, optionsWithoutValues, helmChartPath, releaseName, []string{"templates/nginx-deployment.yaml"})
