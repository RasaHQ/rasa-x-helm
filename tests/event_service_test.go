...
func TestParameters_EventService(t *testing.T) {
	t.Parallel()
	...
	optionsWithValues := &helm.Options{
		SetValues: map[string]string{
			"eventService.podLabels.test_label": "test_value",
		},
	...
	outputWithOptions := helm.RenderTemplate(t, optionsWithValues, helmChartPath, releaseName, []string{"templates/event-service-deployment.yaml"})
	outputWithoutOptions := helm.RenderTemplate(t, optionsWithoutValues, helmChartPath, releaseName, []string{"templates/event-service-deployment.yaml"})
