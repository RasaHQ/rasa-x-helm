...
func TestParameters_Rasa(t *testing.T) {
	t.Parallel()
	...
	optionsWithValues := &helm.Options{
		SetValues: map[string]string{
			"rasa.podLabels.test_label": "test_value",
		},
	...
	outputWithOptions := helm.RenderTemplate(t, optionsWithValues, helmChartPath, releaseName, []string{"templates/rasa-deployments.yaml"})
	outputWithoutOptions := helm.RenderTemplate(t, optionsWithoutValues, helmChartPath, releaseName, []string{"templates/rasa-deployments.yaml"})
